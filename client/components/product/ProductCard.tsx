...
export function ProductCard({ product }: { product: Product }) {
  const price = product.type === "door" ? formatPriceETBPerM2(product.priceETBPerM2) : "Request Quote";
  return (
    <Card className="group overflow-hidden">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {!product.available ? (
          <span className="absolute right-3 top-3 rounded bg-red-600 px-2 py-1 text-xs text-white">Sold Out</span>
        ) : null}
      </div>
      ...
      <CardContent className="pt-0">
        <p className="text-sm text-muted-foreground">{product.specs.join(" · ")}</p>
          ...
          <Dialog>
            ...
            <DialogContent>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <img src={product.images[0]} alt={product.title} className="w-full rounded-md object-cover" />
